...
import { api } from '../config/api'; // Use your existing api config
import { 
...
const Prescription = () => {
  ...
  const fetchNextId = async () => {
    if (editingId) return;
    try {
      const res = await api.get(`/prescriptions/latest-id?type=${tab}`);
  ...
  const handleSave = async (actionType) => {
    setSaving(true);
    try {
      const payload = { ...formData, type: tab, prescriptionId: nextId };
      const res = editingId 
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-semibold">
      ...
      <div className="mx-auto max-w-6xl rounded-[28px] border border-slate-200 bg-white shadow-xl relative">
        ...
        <div className="sticky top-0 z-30 flex bg-slate-900 p-2 shadow-lg">
          {showForm && (
            <button onClick={() => setShowForm(false)} className="mr-2 px-4 text-white hover:bg-slate-800 rounded-xl transition">
              <ArrowLeft size={20}/>
            </button>
          )}
          <button 
            ...
            <div className="animate-in fade-in duration-500">
               <div className="relative mb-4">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-100 text-xs font-bold outline-none focus:border-emerald-500"
                    ...
                    <tbody className="divide-y divide-slate-50 font-bold text-slate-600">
                      {prescriptions.map((item) => (
                        <tr key={item._id} className="hover:bg-slate-50/50">
                          <td className="p-4 text-emerald-600">{item.prescriptionId}</td>
                          <td className="p-4">{item.patient?.fullName}</td>
                          <td className="p-4 text-slate-400 text-xs">{item.doctor?.doctorName}</td>
                          <td className="p-4">
                            <div className="flex items-center justify-center gap-3">
                              <button className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><Printer size={16}/></button>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                  <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-extrabold uppercase text-emerald-600">ID (Auto)</label>
                    <input value={nextId} readOnly className="bg-white border p-2 rounded-xl text-sm outline-none text-slate-400 font-bold" />
                  </div>
                  <div className="flex flex-col gap-1">
                    ...
                    >
                      ...
                      {doctors.map(dr => <option key={dr._id} value={dr._id}>{dr.doctorName}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1">
              ...
              {tab === 'digital' ? (
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {formData.medicines.map((med, idx) => (
                        <div key={idx} className="grid grid-cols-1 md:grid-cols-6 gap-2 bg-slate-50 p-2 rounded-xl border border-dashed border-slate-300">
                          <input 
                            placeholder="Medicine Name" 
                            className="p-2 rounded-lg border text-sm md:col-span-2 outline-none focus:border-emerald-500" 
                            value={med.name}
                            onChange={(e) => {
                          ...
                          />
                          <input placeholder="Dosage" className="p-2 rounded-lg border text-sm" value={med.dosage} />
                          <input placeholder="Frequency" className="p-2 rounded-lg border text-sm" value={med.frequency} />
                          <input placeholder="Duration" className="p-2 rounded-lg border text-sm" value={med.duration} />
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-4">
                  <div className="border-4 border-dashed border-slate-100 rounded-[30px] p-12 text-center flex flex-col items-center gap-4 group hover:border-emerald-200 transition">
                      ...
                      <input type="file" className="hidden" id="fileUp" />
                      <label htmlFor="fileUp" className="cursor-pointer font-bold text-slate-500">
                        Click to upload Prescription Image/PDF
                      </label>
                  </div>
                  <textarea 
